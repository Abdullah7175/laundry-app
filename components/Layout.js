...
export default function Layout({ children, title = 'Nasi` Cleanings Service' }) {
  ...
  return (
    <div className={`${language === 'ar' ? 'rtl' : ''} min-h-screen flex flex-col`}>
      <Head>
        <title>{title}</title>
        <meta name="description" content="Nasi` Cleanings and service for all your bedding needs" />
          ...
          <div className="flex items-center">
            <button onClick={() => router.push('/')} className="flex items-center space-x-2">
              <span className="text-2xl md:text-3xl font-bold">
                {language === 'en' ? 'Nasi` Cleanings' : 'تنظيف الأرز'}
              </span>
            </button>
          </div>
      ...
      <footer className="bg-gray-800 text-white py-10">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            <div>
              <h3 className="text-xl font-bold mb-4">{language === 'en' ? 'Nasi` Cleanings' : 'تنظيف الأرز'}</h3>
          ...
          <div className="mt-12 pt-8 border-t border-gray-700 text-center text-gray-400">
            <p>
              &copy; {new Date().getFullYear()} {language === 'en' ? 'Nasi` Cleanings. All rights reserved.' : 'تنظيف الأرز. جميع الحقوق محفوظة.'}
            </p>
          </div>
        </div>
      </footer>
